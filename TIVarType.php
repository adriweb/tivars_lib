...
//      self::insertType('WindowSettings',      0x0B,   ['8xw']);
    ...
    public static function createFromID($id = -1)
    {
        $instance = new self();
        $instance->id   = $id;
        $instance->exts = self::getExtensionsFromTypeID($id);
        $instance->name = self::getNameFromID($id);
        return $instance;
    }
    ...
    public static function createFromName($name = '')
    {
        $instance = new self();
        $instance->name = $name;
        $instance->id   = self::getIDFromName($name);
        $instance->exts = self::getExtensionsFromName($name);
        return $instance;
    }
}

TIVarType::initTIVarTypesArray();
