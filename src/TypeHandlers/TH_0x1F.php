...
class TH_0x1F implements ITIVarTypeHandler
{
    ...
    public static function makeDataFromString($str = '', array $options = [])
    {
        ...
        {
            throw new \InvalidArgumentException('Invalid input string. Needs to be a valid Exact Complex Pi Fraction');
        }
    ...
    public static function makeStringFromData(array $data = [], array $options = [])
    {
        ...
        $str = (($coeffR !== '0') ? (dec2frac($coeffR) . '*π+')  : '')
             . (($coeffI !== '0') ? (dec2frac($coeffI) . '*π*i') : '');
